---
name: skill-creator
description: Scaffold a new arc skill when a human or community issue asks for one ("add a skill for X", "create a skill that does Y"). Generates correct frontmatter, validates, writes to disk.
...
You are creating a new arc skill **on demand**, in direct response to an explicit request — either from the human creator or from a community issue asking for a new capability.
...
The default if you're unsure: `origin: arc` for arc-decided creations, `origin: creator` for human-driven creations. Never default to `marketplace`.
...
### 5. Pick keywords (only if `origin: arc`)

For `origin: arc` skills, list 3–5 distinctive substrings that would appear in a session's `audit.jsonl` IF this skill were used. skill-evolve uses these to compute `last_used` / `uses` / `wins`.
...
The `[CANDIDATE — unreviewed]` description prefix is critical for `origin: arc` skills — it tells future sessions to treat the skill as experimental until it proves itself (≥2 successful invocations → `status: active`).
...
git commit -m "skill-creator: add $SKILL_NAME (origin: <creator|arc|marketplace>)"
```
...
If you (arc) created this skill in response to a community issue, **also write a journal entry** explaining what was added and why. This is what `communicate` skill is for.

## Pitfalls

- **Don't auto-create skills mid-session without an explicit request.** arc's autonomous self-creation belongs in skill-evolve, which has the right safety gates (recurrence, cooldown, dedup, blast-radius limits). Using skill-creator without a clear human ask is a hard rule violation.
- **Don't set `origin: arc` for skills the human creator explicitly asked for.** Those are `origin: creator` (and probably `core: true`). The reverse is also true — don't set `origin: creator` on something arc decided to make.
- **Don't omit `keywords:` for `origin: arc` skills.** Without keywords, skill-evolve can't compute usage signals; the skill becomes invisible to the scoring loop.
...
- For `origin: arc` skills: `keywords:` has ≥3 entries.
...
- **No browser eval viewer.** arc runs autonomously in CI; no browser. If you need to compare versions, use `git diff`.
