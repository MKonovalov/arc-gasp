...
You are evolving your own skills. This is the only skill that modifies other skills. Treat every cycle with care — what you write here shapes how every future arc session behaves.
...
You may **refine, deprecate, or retire** only skills whose frontmatter declares **`origin: arc`**. Any other value, OR a missing `origin:` field, means the skill is off-limits. This is an allow-list: silence means "don't touch."
...
Today the eligible set is exactly the skills whose SKILL.md declares `origin: arc`:
- `social`
- `family`
- `release`
- any skill you previously spawned (which inherit `origin: arc` from the Create template)

**Defense in depth**: if a skill has `core: true` set, refuse even if `origin: arc` is also somehow present. The two flags should never co-occur, but the conservative move is to honor the deny-flag.
...
ls "${arc_AUDIT_DIR:-/tmp/audit-read/sessions}" 2>/dev/null | tail -30
```

**First-run handling**: if `$arc_AUDIT_DIR` is unset or its directory is empty, the audit-log branch hasn't accumulated evidence yet (this is normal on the first 1–2 cycles). In that case:
...
```bash
# Allow-list: only skills declaring origin: arc are eligible.
...
for d in skills/*/; do
    ...
    grep -q "^origin: arc$" "$d/SKILL.md" || continue
    echo "$name"
done
```
...
- **Failure signals**: tool-call failures in `${arc_AUDIT_DIR}/day-*/audit.jsonl` where the bash command or args reference the skill's domain.
...
Soft delete. Recoverable. If arc invokes the skill's domain again within 3 cycles, you may revive it (move back, reset score to 0.5).
...
- "users will like it"
- "arc will use this skill more" *(no horizon, no signal)*
...
- `pattern_key` recurrence dispersal *falling* over time — arc is internalizing patterns, not re-discovering them
